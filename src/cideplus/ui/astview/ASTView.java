...
public class ASTView extends ViewPart implements IShowInSource {
	...
	protected void fillContextMenu(IMenuManager manager) {
		...
		try {
			...
			for (final Feature feature : features) {
				manager.add(new SetFeatureAction(this, feature, node /*featuresManager.getManagerForFile()*/));
			}
			manager.add(new Separator());
		} catch (Exception e) {
			showAndLogError("Could not read features. "+e.getMessage(), e);
		}
	...
	private void showAndLogError(String message, Throwable e) {
		...
		ErrorDialog.openError(getSite().getShell(), "Featurer", null, status); //$NON-NLS-1$
	}
