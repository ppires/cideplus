...
public class ColorPresentation implements ITextPresentationListener {
	...
	private void checkNode(ASTNode astNode, int offset, int length, CompilationUnitFeaturesManager manager, TextPresentation textPresentation) {
		...
		if(features.size() > 0){
			...
			if(offset <= astNode.getStartPosition() && astNode.getStartPosition() + astNode.getLength() <= offset+length){
				//System.out.println(offset+ ":"+astNode.getStartPosition()+"  ::  "+(astNode.getStartPosition() + astNode.getLength())+":"+(offset+length));
				...
				range.background = new Color(null, combinedRGB);
				range.start = offset;
				if(astNode.getStartPosition() + astNode.getLength() > offset+length){
					range.length = length;
				} else {
					range.length = astNode.getLength() - (offset - astNode.getStartPosition());
				}
				textPresentation.mergeStyleRange(range);
			} else if(offset <= astNode.getStartPosition() && astNode.getStartPosition() + astNode.getLength() > offset+length){
				//System.out.println(offset+ ":"+astNode.getStartPosition()+"  ::  "+(astNode.getStartPosition() + astNode.getLength())+":"+(offset+length));
				StyleRange range = new StyleRange();
				range.background = new Color(null, combinedRGB);
				range.start = astNode.getStartPosition();
